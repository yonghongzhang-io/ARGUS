...
- **Items.** The same five papers as before (`paper_01`, `paper_03`, `paper_07`, `paper_08`,
  ...
  55 cells.
- **Annotators.** Two co-authors of the paper with doctoral-level training in empirical
  finance or economics, neither of them the first author. Each signs the workbook's sign-off
  sheet (name, dates, hours, and four confirmations).
- **Independence and blinding.** Each annotator reads the full PDFs alone, uses no AI tool to
  ...
  returned. As co-authors they have seen the manuscript's aggregate results (for example that
  the system tends to be over-severe); they have not seen any cell-level system output or the
  earlier cell-level labels. This is disclosed in the paper as a limitation.
...
- **Reconciliation.** Cells on which the two differ (applicability or risk) are listed with
  ...
  one final label per cell, or mark the cell *unresolved*; a third co-author who has not seen
  system outputs decides unresolved cells. The first author takes no part.
