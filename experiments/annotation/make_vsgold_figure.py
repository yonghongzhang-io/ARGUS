...
def main() -> None:
    ...
    axA.text(0, 0, f"{len(keys)}\ncells", ha="center", va="center", fontsize=16.0, fontweight="bold")
    axA.set_title("A  Coverage", fontsize=15.4, fontweight="bold", loc="left", color=INK)
    axA.legend(handles=[Patch(facecolor=ACCENT, label=f"answered ({answered})"),
                        Patch(facecolor=C["unknown"], label=f"unknown ({unknown})")],
               fontsize=11.9, loc="center", bbox_to_anchor=(0.5, -0.16), frameon=False, ncol=1)
    ...
    def stack(ax, y, dist, order):
        ...
        for lab in order:
            ...
            if v:
                ...
                ax.text(left + v / 2, y, str(v), ha="center", va="center", fontsize=11.2,
                        color="white", fontweight="bold")
            left += v
    stack(axB, 1, gd, ["low", "medium", "high"])
    stack(axB, 0, ad, ["low", "medium", "high", "unknown"])
    axB.set_yticks([0, 1]); axB.set_yticklabels(["ARGUS", "human gold"], fontsize=12.6)
    axB.set_xlim(0, len(keys)); axB.set_title("B  Risk distribution", fontsize=15.4,
                                              fontweight="bold", loc="left", color=INK)
    axB.spines[["top", "right"]].set_visible(False)
    axB.legend(handles=[Patch(facecolor=C[l], label=l) for l in ["low", "medium", "high", "unknown"]],
               fontsize=10.5, loc="lower center", bbox_to_anchor=(0.5, -0.32), ncol=4, frameon=False)
    ...
    for v, col, lab in [(over, C["high"], "over-severe"), (exact, C["low"], "exact"),
        ...
        if v:
            ...
            axC.text(left + v / 2, 0, str(v), ha="center", va="center", fontsize=12.6,
                     color="white", fontweight="bold")
        left += v
    axC.set_xlim(0, len(ans)); axC.set_ylim(-0.6, 0.6); axC.set_yticks([])
    axC.set_title(f"C  Error direction ({len(ans)} answered)", fontsize=15.4,
                  fontweight="bold", loc="left", color=INK)
    ...
    axC.legend(handles=[Patch(facecolor=C["high"], label=f"over-severe ({over})"),
               ...
               fontsize=10.5, loc="lower center", bbox_to_anchor=(0.5, -0.42), ncol=3, frameon=False)
    ...
    axD.set_xticks(range(len(cols))); axD.set_xticklabels(cols, fontsize=11.9)
    axD.set_yticks(range(len(rows))); axD.set_yticklabels(rows, fontsize=11.9)
    axD.set_xlabel("human gold", fontsize=12.6); axD.set_ylabel("ARGUS", fontsize=12.6)
    mx = max(max(r) for r in M)
    for i, r in enumerate(rows):
        for j, c in enumerate(cols):
            axD.text(j, i, str(M[i][j]), ha="center", va="center", fontsize=12.6,
                     color="white" if M[i][j] > mx * 0.55 else INK)
    axD.set_title("D  Confusion matrix", fontsize=15.4, fontweight="bold", loc="left", color=INK)

    fig.suptitle("ARGUS--LLM vs. adjudicated human gold: diagnostic breakdown",
                 fontsize=16.0, fontweight="bold", color=INK, y=1.0)
