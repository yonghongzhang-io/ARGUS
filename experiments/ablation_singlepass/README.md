> **Historical record (restored from branch `argus-p0-m4-gold-expansion` for the ClimateNLP 2026
> camera-ready).** This directory documents the single-pass variant that **permits `unknown`**
> ("use sparingly") and produced the real-paper contrast in the paper's Appendix C
> (`experiments/real_papers/corpus_results/did_single_pass_risks.csv`: 25 papers, 275 cells,
> one model-chosen `unknown`). The synthetic single-pass numbers in the paper (Tables 1 and 4)
> come from a *different* implementation with a strict low/medium/high schema,
> `experiments/ablations/complete_matrix.py`. The June-2026 figures quoted below (two-stage
> 1.000, single-pass 0.455) no longer reproduce on current serving; see the paper's Appendix F.
