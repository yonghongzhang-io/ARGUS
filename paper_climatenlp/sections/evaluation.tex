...
exactly one known flaw that targets a single
identification dimension, drawn from the flaw taxonomy
(\texttt{flaw\_taxonomy.yaml}; Appendix~\ref{app:flawlit} traces each flaw to
the literature that names it). Because we control which dimension we damaged, we
...
negative-signal phrases, so the circularity cannot silently return. Detection
therefore cannot be achieved by string matching.
