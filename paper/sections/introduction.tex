...
Difference-in-differences (DID) is one of the most widely used research designs in
empirical economics, and a particularly visible one in environmental policy, our
motivating case.
...
both hopeless and, we think, the wrong target. We ask instead a question that can
be grounded in the paper itself: \emph{is each identification assumption supported
by the evidence the study reports, and where is that support weakest?}
...
confined to two scoped sub-tasks. Because the true effect is unavailable as ground truth, we validate
...
while preserving high-risk recall. ARGUS is thus best read as a high-sensitivity
screening tool that flags dimensions for expert review rather than an autonomous
arbiter of causal truth.
...
local ground truth} for identification auditing (it needs no access to the
true effect), showing that adequacy reasoning catches flawed-but-present evidence
that keyword presence cannot, with a real-paper deployment on 27 DID papers that
relocates the bottleneck to \emph{evidence grounding} and separates retrieval
failure from substantive high risk; an \emph{architecture ablation} against a
same-model single-pass baseline shows the bounded design raises synthetic flaw
detection from $0.455$ to $1.000$ and, on real papers, preserves an explicit
\texttt{unknown} state ($38.9\%$ vs $0.4\%$) that keeps evidence-grounding
uncertainty visible rather than committing a guess (\S\ref{sec:results}); and
\textbf{(iv)}~a \textbf{pilot human-gold calibration loop} (5 papers, 55
dimension-level cells) in which disagreement is largely
design-applicability-driven: comparing ARGUS to an adjudicated gold reveals a
one-directional over-severity bias (more severe than gold in 28/33 answered cells,
less severe in none), and an applicability-aware, anchor-based calibration layer
reduces it while preserving high-risk recall. Expert annotation can therefore both
evaluate and calibrate an identification audit. The human-gold analysis is
pilot-scale; larger expert-labelled corpora are future work.

\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{figures/figure1}
  \caption{Overview of ARGUS. Step~1 converts an empirical DID paper into an
  ...
  calibrate audit severity. ARGUS flags dimensions for expert review; it does not
