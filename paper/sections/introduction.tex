...
the true effect (\S\ref{sec:evaluation}). (4)~An honest deterministic baseline and
a \emph{negative result}: once the leakage that makes naive evaluation circular is
removed, keyword-based detection collapses from a trivial $1.000$ to $0.182$, and
the baseline is systematically blind to threats where flawed-but-plausible
evidence is present (\S\ref{sec:results}). This quantifies the reasoning gap that
the remainder of the doctoral programme is designed to close.
