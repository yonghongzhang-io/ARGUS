...
Three assessors are selectable:
  - "keyword":     the deterministic baseline (positive/negative signal scoring) over
                   the stage-2 evidence bundle (paragraph chunks from evidence_search).
  - "llm":         a model that judges evidence ADEQUACY. It does NOT consume the
                   stage-2 bundle: it re-retrieves at section level (retrieve_sections)
                   and gates the candidates, so keyword-vs-llm is an END-TO-END pipeline
                   comparison, not a swap of the judgement policy over shared evidence.
  - "single_pass": whole paper + all dimensions in ONE call, no retrieval or gate. This
                   variant may answer `unknown` ("use sparingly") and produced the
                   real-paper contrast; the synthetic ablation in
                   experiments/ablations/complete_matrix.py uses a separate strict
                   low/medium/high schema with no `unknown`.
...
def _assess_single_pass(skeleton: dict[str, Any], paper: dict[str, Any]) -> dict[str, Any]:
    """Single-pass ablation: one whole-paper call over all dimensions (no retrieval/gate).

    Same model, rubric, and adequacy standard as ``_assess_llm``; there is no
    per-dimension retrieval or relevance gate, and the model may answer ``unknown``.
    """
